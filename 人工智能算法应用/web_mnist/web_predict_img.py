...
def hello_world():
    if request.method == 'POST':
        ...
        change_img = cv2.imread(upload_path)
        change_img = cv2.putText(change_img, str(label), (0,0), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255))
        label_path = "../static/images/label.jpg"
        cv2.imwrite(label_path, change_img)
        # return render_template('upload_ok.html',path = show_path, label = label)
        return render_template('two_col.html',path = show_path, label = label)
        # return "上传成功"
    else:
        print('get')
