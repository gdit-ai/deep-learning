...
def stitch_img(small_img, input_img):
    h,w,c = input_img.shape
    size = (160, 160)
    small_img = cv2.resize(small_img, size, interpolation=cv2.INTER_AREA)

    im1 = Image.fromarray(cv2.cvtColor(small_img, cv2.COLOR_BGR2RGB))
    im2 = Image.fromarray(cv2.cvtColor(input_img, cv2.COLOR_BGR2RGB))
...
while True:
    ...
    if face_flag:
        ...
        frame = stitch_img(src_img, frame)
